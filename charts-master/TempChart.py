...
class TempChart(QWidget):
    def __init__(self, parent):
        ...
        def updateLineColor(self):
            # Überprüfen des aktuellen Werts auf der Y-Achse
            current_y = self.series.at(0).y()
            if current_y < 15:
                # Wenn der Wert unter 1 liegt, wird die Linie rot
                self.series.setColor(QColor("red"))
            else:
                # Wenn der Wert 1 oder größer ist, wird die Linie schwarz
                self.series.setColor(QColor("black"))
